...
%   Curso                 = doutorado/mestrado/monografia
%   Situação do trabalho  = pre-defesa/pos-defesa (exceto para qualificação)
%   Versão para impressão = impressao
% caso queira usar para TCC utilizar a seguinte configuração e mudar o curso 
% \documentclass[tcc]{packages/icmc}
...
\tituloPT{Modelo de teses e dissertações em LaTeX da EESC}
\tituloEN{Model of theses and dissertations in LaTeX of the EESC}
\autor[Pinheiro, J. M. H.]{João Manoel Herrera Pinheiro}
\genero{M} % Gênero do autor (M = Masculino / F = Feminino)
\orientador[Orientador]{Prof. Dr.}{Marcelo Becker}
%\coorientador{Prof. Dr.}{Fulano de Tal}
%\curso{DEPARTAMENTO}
% alterar o arquivo icmc.cls  na linha 488, coloque as informações do seu PROGRAMA de pós, tais como o nome completo em pt e ingles, assim como as áreas de concentração em ambos os idiomas
% template foi feito apenas para o departamento de eng mecanica e eletrica
% alguns exemplos, SEM_aero, SEM_din_mec, SEM_projeto, SEM_termo, SEL_din
\curso{SEM_din_mec}
\data{17}{10}{2023} % Data do depósito
...
\textoresumo[brazil]{
    Este trabalho é um breve modelo  para a escrita de monografias de qualificação, dissertações e teses utilizando o ambiente \LaTeX, de acordo com as normas exigidas pela Escola de Engenharia de São Carlos (EESC), da Universidade de São Paulo (USP). Para a confecção deste modelo foi utilizado a última versão (1.9.6) do pacote de classes \textit{abnTeX2} que segue as normas da Associação Brasileira de Normas Técnicas. A elaboração de uma monografia, dissertação ou tese pode ser feita sobrescrevendo o conteúdo deste modelo. 
...
% conter no mínimo 1 e no máximo 5 palavras-chave
\textoresumo[english]{
    This paper is a brief model for writing qualification monographs, dissertations and thesis using \LaTeX environment, by the standards required by the São Carlos School of Engineering (EESC), University of São Paulo (USP). For making this model, the latest version (1.9.6) \textit{abnTeX2} classes package was used. This package follow the rules of the Brazilian Association of Technical Standards. Drafting a monograph, dissertation or thesis can be done by overwriting the contents of this model.
